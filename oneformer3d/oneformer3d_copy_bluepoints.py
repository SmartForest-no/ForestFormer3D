...
class ScanNetOneFormer3DMixin:
    ...
    def predict_by_feat_instance(self, out, superpoints, score_threshold):
        ...
        sp_score_thr = float(os.environ.get("FF3D_SP_SCORE_THR", self.test_cfg.sp_score_thr))
        mask_pred = mask_pred_sigmoid > sp_score_thr
...
class ForAINetV2OneFormer3D(Base3DDetector):
    ...
    def pred_inst_sem(self, pred_masks, pred_scores,
        ...
        # Optionally skip stuff filtering for no-ground point clouds.
        if os.getenv("FF3D_NO_STUFF_FILTER") != "1":
    ...
    def pred_inst_sem_test(self, pred_masks, pred_scores,
        ...
        disable_suspend_filter = os.environ.get("FF3D_DISABLE_SUSPEND_FILTER", "0") == "1"
        suspend_z_margin = float(os.environ.get("FF3D_SUSPEND_Z_MARGIN", 5.0))
        if not disable_suspend_filter:
            for i in range(mask_pred.size(0)):
                ...
                if z_values.numel() > 0 and z_values.min().item() > ground_z_max + suspend_z_margin:
                    scores[i] = 0
        ...
        # npoint_thr
        mask_pointnum = mask_pred.sum(1)
        npoint_thr = int(os.environ.get("FF3D_NPOINT_THR", self.test_cfg.npoint_thr))
...
class ForAINetV2OneFormer3D_XAwarequery(Base3DDetector):
    ...
    #def predict_rm_outputpoints(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        #is_test = True
        #if is_test:
        print(f"[FF3D] lidar_path: {lidar_path}")
        if 'test' in lidar_path:
            ...
            output_path = self.output_path or os.environ.get(
                "FF3D_OUTPUT_PATH",
            ...
            for region_idx, region in enumerate(tqdm(regions, desc="Processing regions")):
                ...
                if tree_indices.numel() > 1:
                    ...
                    edge_threshold = self.radius - edge_margin  # Distance to cylinder edge threshold
            ...
            t11 = time.time()     
            # Remove instances with fewer than 10 points  
            if not disable_small_instance_filter:
                uniq, cnt = np.unique(all_pre_ins, return_counts=True)
                to_kill   = np.isin(all_pre_ins, uniq[(cnt < min_inst_points) & (uniq != -1)])
